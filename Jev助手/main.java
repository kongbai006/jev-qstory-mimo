// Jev 助手（Mimo 版）v1.7.2 正式版 — QStory 可加载插件
// 功能：读取消息 → 秒弹「快速意图+危险+情绪百分比+思考中」→ （可选）Mimo 返回后原地更新完整候选 → 点发送才发出
// v1.7.2：删除设置弹窗里重复的"默认关系描述"字段（关系设置统一走菜单「本会话关系设置」）；
//          字段标签改为详细说明；框内提示改示例文字；密钥框不预填、留空不改动现有密钥
...
public void onMsg(MessageData msg) {
    ...
    // 群聊触发条件
    if (msg.IsGroup) {
        String trigger = config.optString("groupTrigger", "at");
        boolean atMe = msg.mAtList != null && msg.mAtList.contains(myUin);
        diag("群聊 触发方式=" + trigger + " 被@=" + atMe);
        if (trigger.equals("at") && !atMe) return;
        if (atMe) {
            content = content.replace("[AtQQ=" + myUin + "]", "").trim();
        }
    }
    ...
    act.runOnUiThread(new Runnable() {
        public void run() {
            try {
                ...
                final android.widget.EditText etGroup = makeField(act, "群聊触发方式（at=被@机器人才分析；all=群里每条消息都分析）", config.optString("groupTrigger", "at"), 2, "示例：at", form);
                ...
                btnSave.setOnClickListener(new android.view.View.OnClickListener() {
                    public void onClick(android.view.View v) {
                        try {
                            ...
                            config.put("groupTrigger", etGroup.getText().toString().trim());
