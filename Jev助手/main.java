// Jev 助手（Mimo 版）v1.9 — QStory 可加载插件
// 功能：私聊消息 → 秒弹「快速意图+危险+情绪百分比+思考中」→ （可选）Mimo 返回后原地更新完整候选 → 点发送才发出
// v1.9：第二轮提速（max_tokens 默认 500、精简请求）；本地判断沿用意图/危险/情绪三分类
    ...
    act.runOnUiThread(new Runnable() {
        public void run() {
            try {
                ...
                // ② API 与模型区
                sectionTitle(act, form, "② API 与模型（任意 OpenAI 兼容）");
                final android.widget.EditText etUrl = makeField(act, "API 服务地址（/v1 结尾；默认 MiMo，可改 DeepSeek/通义/智谱/OpenAI）", config.optString("baseUrl", "https://api.xiaomimimo.com/v1"), 2, "示例：https://api.deepseek.com/v1", form);
                final android.widget.EditText etKey = makeField(act, "API 密钥（任意厂商 sk- 开头；留空则不修改现有密钥）", "", 4, "示例：sk-xxxxxxxx", form);
                final android.widget.EditText etModel = makeField(act, "模型名称（默认 mimo-v2.6-flash，可改 deepseek-chat / qwen-plus）", config.optString("model", "mimo-v2.6-flash"), 2, "示例：deepseek-chat", form);
...
JSONObject buildPayload(String key, String convo) throws Exception {
    ...
    payload.put("max_tokens", config.optInt("maxTokens", 500));
    return payload;
}
